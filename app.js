const App = (() => {
  ...
  // ════ RIGHTS SUB-PAGE CONTROLS ════
  function showRightsList() {
    const dash = document.getElementById('view-rights-dashboard');
    ...
    if (dash && main) {
      ...
      document.querySelector("#screen-rights .content").scrollTop = 0;
    }
  }

  function backToRightsDashboard() {
    const dash = document.getElementById('view-rights-dashboard');
    const main = document.getElementById('view-rights-main');
    const quiz = document.getElementById('view-rights-quiz');
    if (dash && main) {
  ...
  function updateRightsSummary() {
    ...
    // Updates for dashboard numbers
    const dashboardRightsCount = document.getElementById("dashboardRightsCount");
    ...
    if (answered < 3) {
      ...
      if (dashboardRightsCount) dashboardRightsCount.textContent = "0";
      return;
    }
    ...
    if (dashboardRightsCount) dashboardRightsCount.textContent = `${rights.length}`;
  ...
  function toggleMic() {
    ...
    recognition.onend = () => {
      ...
      if (transcript && transcript !== "กำลังฟังอยู่...") {
        ...
        setTimeout(() => askAI(transcript, answerEl, micStatus), 900);
      } else {
  ...
  async function askAI(transcript, answerEl, micStatus) {
    try {
      const history = load("voiceHistory", []);
      ...
      const answer = data.answer || "ขออภัย ระบบยังไม่สามารถตอบได้";

      const newHistory = load("voiceHistory", []);
      newHistory.unshift({ q: transcript, a: answer, ts: Date.now() });
      if (newHistory.length > 20) newHistory.pop();
      save("voiceHistory", newHistory);
      ...
      if (state.autoSpeak) {
        setTimeout(() => speakText(answer), 250);
      }

      showToast("✅ AI ตอบแล้ว");
    } catch (error) {
      console.error(error);
      answerEl.innerHTML = `
        <div class="voice-answer-card">
          <h3>เกิดข้อผิดพลาด</h3>
          <p>ตอนนี้ยังเชื่อมต่อ AI ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง</p>
        </div>
      `;
      micStatus.textContent = "แตะเพื่อพูดอีกครั้ง";
      showToast("❌ เชื่อมต่อ AI ไม่สำเร็จ");
    }
  }
